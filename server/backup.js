...
// 3) скопировать нужный файл из data/backups/ поверх data/monolit.db,
...
const dbPath = path.join(__dirname, '..', 'data', 'monolit.db');
...
const backupPath = path.join(backupDir, `monolit-${timestamp}.db`);
...
const files = fs.readdirSync(backupDir)
  .filter(f => f.startsWith('monolit-') && f.endsWith('.db'))
  .sort();
